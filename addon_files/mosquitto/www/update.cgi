#!/bin/tclsh
#
#   Self-update backend for the settings page (ROADMAP task 5).
#
#     ?cmd=start&sid=@...@[&force=1]   start bin/mosquitto-update detached
#     ?cmd=status                      current state (JSON, written by the worker)
#     ?cmd=log&sid=@...@               the worker's log (text)
#     ?cmd=reset&sid=@...@             forget a finished run
#
#   status needs no session: the page polls it every second for a while and
#   it exposes nothing but phase and version numbers. Everything else
#   triggers or reads privileged things and needs the CCU session.
#

source ../lib/querystring.tcl

set STATE_DIR /tmp/mosquitto-update
set STATE $STATE_DIR/state.json
set WORKER /usr/local/addons/mosquitto/bin/mosquitto-update
...
if {$cmd == "start"} {
    ...
    set fp [open $STATE w]
    puts $fp {{"phase":"starting","message":"Update wird gestartet ...","ts":0}}
    close $fp
    set args ""
    ...
    puts {{"phase":"starting","message":"Update wird gestartet ..."}}
} elseif {$cmd == "reset"} {
    if {![running]} {
