# Roadmap archive

Completed and closed roadmap items — one file per item, named after its
task number (e.g. `task-1.md`). Open items live in
[../ROADMAP.md](../ROADMAP.md); task numbers are stable and never
reused, so an archived item stays findable from the roadmap's contents
index (marked ✅ there, linking here).

## Contents

- [task-1.md](task-1.md) — Repository and tooling modernization ✅ 2026-09-05
- [task-2.md](task-2.md) — Build from source in Alpine containers for three architectures ✅ 2026-09-05 (3.3 MB per architecture)
- [task-3.md](task-3.md) — Addon runtime: service script, installer, migration ✅ 2026-09-05
- [task-4.md](task-4.md) — Configuration web UI ✅ 2026-09-05 (no frameworks, Tcl 8.2 compatible CGIs)
- [task-5.md](task-5.md) — Self-update from the settings page ✅ 2026-09-05
- [task-6.md](task-6.md) — Continuous integration and end-to-end test ✅ 2026-09-05
- [task-7.md](task-7.md) — Version scheme and automatic releases ✅ 2026-09-05
- [task-8.md](task-8.md) — Hardware verification on the lab systems ✅ 2026-09-05 (CCU3 incl. migration from 1.5.8+4, OpenCCU x86_64 and aarch64)
- [task-9.md](task-9.md) — Documentation ✅ 2026-09-05
- [task-10.md](task-10.md) — First release ✅ 2026-09-05
- [task-11.md](task-11.md) — Bridge setup in the web UI ✅ 2026-09-05
- [task-12.md](task-12.md) — TLS listeners in the default configuration ✅ 2026-09-05
- [task-13.md](task-13.md) — CCU firewall: show blocked ports, open them from the UI ✅ 2026-09-05
- [task-15.md](task-15.md) — Persistence on a USB stick, configurable in the UI ✅ 2026-09-05
- [task-16.md](task-16.md) — Tests in Node.js: unit, CGI integration, web UI end to end, coverage ✅ 2026-09-05
- [task-17.md](task-17.md) — Per-listener authentication ✅ 2026-09-05 (`listener_allow_anonymous`, released as 2.1.2+1)
- [task-19.md](task-19.md) — openccu-lite: run unchanged on a CCU without ReGaHSS ✅ 2026-09-06 (released as 2.1.2+2)
- [task-20.md](task-20.md) — Every aarch64 release so far was an armv7l package ✅ 2026-09-06 (released as 2.1.2+3)
